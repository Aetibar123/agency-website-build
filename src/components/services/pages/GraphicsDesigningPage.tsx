          ...
          <Grid container spacing={10}>
            ...
            <Grid size={{ xs: 12, md: 8 }}>
              {service.detailedContent?.map((content, idx) => (
                <Box key={idx} sx={{ mb: 8 }}>
                  <Typography variant="h3" sx={{ fontWeight: 800, mb: 4, color: 'white' }}>
                    {content.heading}
                  </Typography>
                  {content.paragraphs.map((p, pIdx) => (
                    <Typography key={pIdx} variant="body1" sx={{ color: 'rgba(255,255,255,0.7)', fontSize: '1.15rem', lineHeight: 2, mb: 3 }}>
                      {p}
                    </Typography>
                  ))}
            ...
            <Grid size={{ xs: 12, md: 4 }}>
              <Box sx={{ position: 'sticky', top: 120 }}>
                <Card sx={{ bgcolor: 'rgba(255,255,255,0.03)', color: 'white', borderRadius: 6, p: 4, border: '1px solid rgba(255,255,255,0.1)', backdropFilter: 'blur(10px)', mb: 4 }}>
                  ...
                  <Typography variant="h5" sx={{ fontWeight: 800, mb: 4 }}>Design Methodology</Typography>
                  <Box sx={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                    {service.features.map((feature, idx) => (
                      <Box key={idx}>
                        <Typography variant="subtitle1" sx={{ fontWeight: 700, color: 'primary.light', mb: 0.5 }}>{feature.title}</Typography>
                        <Typography variant="body2" sx={{ color: 'rgba(255,255,255,0.6)' }}>{feature.desc}</Typography>
                      </Box>
                    ))}
                  </Box>
                </Card>
                ...
                <Box sx={{ p: 4, borderRadius: 6, bgcolor: 'rgba(110,65,226,0.1)', border: '1px solid rgba(110,65,226,0.2)' }}>
                   <Typography variant="subtitle2" sx={{ fontWeight: 700, mb: 2, color: 'primary.light', textTransform: 'uppercase' }}>Design Stack</Typography>
